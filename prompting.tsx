...
export function PromptingIsAllYouNeed({ 
  ...
  // --- Mount/Resize effect: only runs once ---
  useEffect(() => {
  ...
  // --- Game loop and mouse event effect ---
  useEffect(() => {
    const canvas = canvasRef.current
    if (!canvas) return
    const ctx = canvas.getContext("2d")
    ...
    if (gameState === 'playing') {
      ...
      drawGame() // Draw the last frame for idle/gameOver
    }
    return () => {
      canvas.removeEventListener("mousemove", handleMouseMove)
      cancelAnimationFrame(animationId)
    }
  }, [gameState, score, settings, isEmbedded])

  // --- Logic functions: must be in scope for all effects ---
  ...
  const updateGame = () => {
    ...
    if (ball.x + ball.radius > canvasRef.current!.width) {
      ball.dx = -ball.dx
      ball.x = canvasRef.current!.width - ball.radius
